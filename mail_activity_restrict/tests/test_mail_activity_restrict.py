...
class TestMailActivityRestrict(TransactionCase):
    ...
    def test_technical_write_by_other_user_is_allowed(self):
        """Writing a field the user does not act on is not restricted.

        Other addons stamp bookkeeping fields on activities while acting for
        the user; ``mail_activity_reminder`` does it from ``action_notify()``,
        which core calls inside ``create()``. Blocking those writes breaks
        every addon that touches an activity in passing.
        """
        activity = self._create_activity(
        ...
        activity.with_user(self.user_manager).write(
            {"previous_activity_type_id": self.activity_type_unrestricted.id}
        )

        self.assertEqual(
            activity.previous_activity_type_id, self.activity_type_unrestricted
        )

    def test_technical_write_context_cannot_bypass_restriction(self):
        """A forged context key does not lift the restriction."""
        activity = self._create_activity(
            self.activity_type_restricted, self.user_assigned
        )

        with self.assertRaises(AccessError):
            activity.with_user(self.user_manager).with_context(
                mail_activity_restrict_technical_write=True
            ).write({"summary": "Forged"})
